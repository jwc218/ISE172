...
Last update by Martin Takac 2017
'''
...
def createGraph():
    ...
    while sideLength<=60:
        startTime=time()
        life(sideLength,sideLength,40)
        taskTime=time()-startTime
        size.append(sideLength)
        timing.append(taskTime)
        sideLength+=10
    plt.plot(size,timing,'ro')
    plt.axis([0,70,0,120])
    plt.show()
    popt, pcov = curve_fit(exponenial_func, size, timing)
    space=np.linspace(0, 70, 1)
    plt.plot(size,timing,'ro')
    plt.axis([0,70,0,120])
    plt.plot(space, exponenial_func(size, *popt), 'r-')
    plt.show()
    print "running time curve mimics an x squared function with respect to graph size"
...
if __name__ == "__main__": 
    ...
    if len(argv) == 3:
        ...
        cProfile.run('life(20, 20, 20)', 'cprof.out')
