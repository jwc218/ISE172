...
from stacks_student import Stack
    ...
    def draw_maze(self):
        ...
        for x in range(self.dimension['y']):
            for y in range(self.dimension['x']):
                ...
                pygame.draw.rect(self.background,self.colors[self.maze[(y,x)]],rectangle) 
            ...
            for j in range(-1, 2, 1):
                if (0 <= self.location[1] + i < self.dimension['x'] and 
                    ...
                    pygame.draw.rect(self.background, 
                                     self.colors[self.maze[(self.location[0]+j,self.location[1]+i)]], rectangle) 
...
if __name__ == '__main__':
    for i in range(0,5):
        maze = Maze(100, 100, 2, 0.5)
    #maze.set_mode(GRAPHICAL_LIMITED)
        maze.set_mode(GRAPHICAL_FULL)
#    maze.set_mode(SILENT)
        path = maze.solve()
        print path.count
        print maze
        print path
